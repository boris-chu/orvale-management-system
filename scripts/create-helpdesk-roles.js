...
// Step 1: Create Helpdesk Team first
...
// New permissions for Helpdesk operations
const newPermissions = [
    'ticket.assign_cross_team',      // Assign tickets between different teams
    'queue.view_escalated',          // View the escalated/helpdesk queue
    'ticket.manage_escalated',       // Full management of escalated tickets
    'ticket.reassign_any_team',      // Reassign tickets from any team to any other team
    'queue.access_helpdesk'          // Permission to access helpdesk queue
];
        ...
        db.run(
            ...
            function(err) {
                ...
                if (usersCreated === totalUsers) {
                    console.log(`\n🎉 Successfully created ${totalRoles} roles and ${totalUsers} users!`);
...
function verifySetup() {
    ...
    db.all(
        ...
        (err, roles) => {
            ...
            // Check users
            db.all(
                `SELECT username, display_name, role FROM users WHERE username IN ('jane.smith', 'john.smith')`,
                (userErr, users) => {
                    if (userErr) {
                        console.error('❌ Error verifying users:', userErr);
                    } else {
                        console.log(`👥 Verified ${users.length} helpdesk users created`);
                        users.forEach(user => console.log(`  ➤ ${user.display_name} (${user.username}) - ${user.role}`));
                    }
...
// Close the createRoles function
    });
}

// Step 3: Create users (already defined above as createUsers function)
