...
export const getUserPermissions = async (user: User): Promise<string[]> => {
    try {
        ...
        console.log('🔒 SECURITY: No fallback permissions - failing securely');
        
        // SECURITY: Fail securely - return empty permissions if database is compromised
        // This ensures that database errors don't accidentally grant admin access
        return [];
    }
};
