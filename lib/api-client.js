...
class OrvaleAPIClient {
    ...
    async login(credentials) {
        const result = await this.makeRequest('auth', 'login', credentials, false);
    ...
    async getMaintenanceStatus() {
        return await this.makeRequest('system', 'get_maintenance_status', {}, false);
    }

    // === SYSTEM INFORMATION ===
    async getSystemInfo() {
        return await this.makeRequest('system', 'get_system_info', {}, false);
    }
    ...
    async createDirectMessage(targetUsername) {
        return await this.makeRequest('chat', 'create_direct_message', { target_username: targetUsername });
    }

    async getDirectMessages() {
        return await this.makeRequest('chat', 'get_direct_messages');
    }
    ...
    async getPersonalDashboard() {
        // Personal dashboard has its own dedicated endpoint
        try {
            const headers = {
                'Content-Type': 'application/json'
            };
            ...
            console.error('API Error [dashboard.personal]:', error);
            return {
                success: false,
                error: error.message
            };
    ...
    async getTicketAttachment(ticketId) {
        return await this.makeRequest('tickets', 'get_attachments', { ticket_id: ticketId });
    }
    ...
    async getAssignableUsers(teamId) {
        return await this.makeRequest('utilities', 'get_assignable_users', { team_id: teamId });
    }
    ...
    async getCurrentUser() {
        ...
        // Fix the nested response structure for consistency
        if (result.success && result.data && result.data.data && result.data.data.user) {
            // Extract the user from the nested structure and return it at the top level
            return {
                success: true,
