...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get total tickets generated by user
    const totalTicketsResult = await queryAsync(
      'SELECT COUNT(*) as count FROM user_tickets WHERE employee_number = ?',
      [username]
    );
    ...
    const currentMonthTickets = await queryAsync(
      'SELECT COUNT(*) as count FROM user_tickets WHERE employee_number = ? AND submitted_at >= ?',
      [username, currentMonthStart.toISOString()]
    );

    const lastMonthTickets = await queryAsync(
      'SELECT COUNT(*) as count FROM user_tickets WHERE employee_number = ? AND submitted_at >= ? AND submitted_at <= ?',
      [username, lastMonthStart.toISOString(), lastMonthEnd.toISOString()]
    );
    ...
    const weeklyTickets = await queryAsync(
      'SELECT COUNT(*) as count FROM user_tickets WHERE employee_number = ? AND submitted_at >= ?',
      [username, weekStart.toISOString()]
    );
    ...
    const dailyActivity = await queryAsync(`
      SELECT 
        ...
        COUNT(*) as count
      FROM user_tickets 
      WHERE employee_number = ? AND submitted_at >= ?
      GROUP BY DATE(submitted_at)
      ORDER BY date
    `, [username, thirtyDaysAgo.toISOString()]);
